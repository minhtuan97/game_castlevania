...
{
	float dx_entry, dx_exit, tx_entry, tx_exit, dy_entry, dy_exit, ty_entry, ty_exit;
	float t_entry, t_exit;

	t = -1.0f; //khong co va cham
	nx = ny = 0;

	//dx, dy là khoảng cách di chuyển trong 1 frame
	...
	if (br < sl || bl > sr || bb < st || bt > sb) return;//k va cham
	//DebugOut(L"CO va cham trong sweptaabb\n");
	...
	if ((tx_entry < 0.0f && ty_entry < 0.0f) || tx_entry > 1.0f || ty_entry > 1.0f) return;//khong va cham
